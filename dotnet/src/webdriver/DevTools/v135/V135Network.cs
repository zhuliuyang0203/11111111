// <copyright file="V135Network.cs" company="Selenium Committers">
...
/// <summary>
/// Class providing functionality for manipulating network calls using version 135 of the DevTools Protocol
/// </summary>
public class V135Network : DevTools.Network
{
    private FetchAdapter fetch;
    private NetworkAdapter network;

    /// <summary>
    /// Initializes a new instance of the <see cref="V135Network"/> class.
    /// </summary>
    ...
    public V135Network(NetworkAdapter network, FetchAdapter fetch)
    {
    ...
    public override async Task ContinueWithAuth(string requestId, string? userName, string? password)
    {
        await fetch.ContinueWithAuth(new ContinueWithAuthCommandSettings()
        {
            ...
            {
                Response = V135.Fetch.AuthChallengeResponseResponseValues.ProvideCredentials,
                Username = userName,
                Password = password
            }
        }).ConfigureAwait(false);
    ...
    public override async Task CancelAuth(string requestId)
    {
        await fetch.ContinueWithAuth(new ContinueWithAuthCommandSettings()
        {
            ...
            {
                Response = V135.Fetch.AuthChallengeResponseResponseValues.CancelAuth
            }
        }).ConfigureAwait(false);
    }
